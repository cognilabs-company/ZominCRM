...
export const Card: React.FC<CardProps> = ({ children, className = '', title, action, accent = 'none' }) => {
  return (
    ...
    `}>
      {(title || action) && (
        <div className="px-6 py-4 border-b border-light-border dark:border-white/6 flex justify-between items-center">
          {title && (
            <h3 className="text-base font-semibold text-light-text dark:text-white/90 tracking-tight">
              {title}
            </h3>
          )}
