...
const Leads: React.FC = () => {
  ...
  const statusLabel = (status: LeadStatus) => {
    const labels: Record<LeadStatus, Record<'en' | 'ru' | 'uz', string>> = {
      NEW: { en: 'New', ru: 'Novyy', uz: 'Yangi' },
      QUALIFIED: { en: 'Qualified', ru: 'Kvalifitsirovan', uz: 'Saralangan' },
      CONVERTED: { en: 'Converted', ru: 'Konvertirovan', uz: 'Konvert qilingan' },
      LOST: { en: 'Lost', ru: 'Poteryan', uz: "Yo\'qotilgan" },
    };
  ...
  const loadData = async () => {
    try {
      ...
      const message = e instanceof Error ? e.message : tr('Failed to load leads', 'Failed to load leads', "Lidlarni yuklab bo'lmadi");
      setError(message);
      toast.error(message);
    } finally {
      setLoading(false);
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setEditing(null);
      await loadData();
      toast.success(editing ? tr('Lead updated.', 'Lead updated.', 'Lid yangilandi.') : tr('Lead created.', 'Lead created.', 'Lid yaratildi.'));
    } catch (e) {
      const message = e instanceof Error ? e.message : tr('Failed to save lead', "Lidni saqlab bo\'lmadi", "Lidni saqlab bo\'lmadi");
      setError(message);
      toast.error(message);
    } finally {
      setSaving(false);
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-navy-900/50 text-xs uppercase text-gray-500 dark:text-gray-400 border-b border-light-border dark:border-navy-700">
                <th className="px-6 py-4 font-semibold">{tr('Lead ID', 'Lead ID', 'Lid ID')}</th>
                <th className="px-6 py-4 font-semibold">{t('nav_clients')}</th>
                <th className="px-6 py-4 font-semibold">{tr('Source', 'Source', 'Manba')}</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={6} className="py-10 text-center text-gray-400">
                    {tr('Loading leads...', 'Loading leads...', 'Lidlar yuklanmoqda...')}
                  </td>
                </tr>
              ) : leads.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-10 text-center text-gray-400">
                    {tr('No leads found.', 'No leads found.', 'Lidlar topilmadi.')}
                  </td>
                </tr>
              ) : (
                leads.map((lead) => {
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="inline-flex items-center gap-1">
                          <button
                            ...
                            title={tr('Go to chat', 'Go to chat', "Chatga o'tish")}
                          >
                          ...
                          <button
                            ...
                            title={tr('Edit lead', 'Edit lead', 'Lidni tahrirlash')}
                          >
                            <Edit2 size={16} />
                          </button>
                        </div>
      ...
      <Modal
        ...
        title={editing ? tr('Edit Lead', 'Edit Lead', 'Lidni tahrirlash') : tr('New Lead', 'New Lead', 'Yangi lid')}
        footer={null}
      >
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{tr('Source', 'Source', 'Manba')}</label>
            <input
              name="source"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{tr('Notes', 'Notes', 'Izohlar')}</label>
            <textarea
              name="notes"
