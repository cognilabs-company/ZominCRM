...
const Orders: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <button onClick={() => setIsFilterOpen((value) => !value)} className={`flex items-center gap-1.5 px-3 py-1.5 border text-xs rounded-lg transition-colors ${isFilterOpen ? 'bg-blue-50 border-primary-blue text-primary-blue dark:bg-navy-700 dark:border-blue-500 dark:text-blue-400' : 'bg-white dark:bg-navy-800 border-light-border dark:border-navy-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-navy-700'}`}>
        <Search size={14} />
        {t('filters')}
      </button>
      <button onClick={() => setIsCreateOpen(true)} className="px-3 py-1.5 bg-primary-blue hover:bg-blue-700 text-white rounded-lg text-xs font-medium transition-colors flex items-center gap-1.5">
      ...
      <Modal
        ...
        footer={<div className="flex gap-2 w-full justify-end"><button onClick={closeOrderDetails} className="px-3 py-1.5 rounded-lg text-xs border border-light-border dark:border-navy-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-navy-700 transition-colors">{t('cancel')}</button></div>}
      >
        {selectedOrder ? (
          <div className="space-y-6">
      ...
      >
        <form className="space-y-6" onSubmit={handleCreateOrder}>
          ...
          <button type="button" onClick={closeCreateModal} className="px-3 py-1.5 rounded-lg text-xs border border-light-border dark:border-navy-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-navy-700 transition-colors">{t('cancel')}</button>
          <button disabled={createLoading} type="submit" className="px-3 py-1.5 rounded-lg text-xs font-medium bg-primary-blue text-white hover:bg-blue-700 transition-colors disabled:opacity-50">{createLoading ? tr('Saving...', 'Saving...', 'Saqlanmoqda...') : tr('Create order', 'Create order', 'Buyurtma yaratish')}</button>
          </div>
        </form>
      </Modal>
    </div>
