    ...
    >
              ...
              <button
                ...
                className={`w-full rounded-[24px] border p-4 text-left transition ${
                  isSelected
                    ...
                    : 'border-slate-200 bg-white text-slate-950 hover:border-slate-300 hover:shadow-[0_18px_36px_rgba(15,23,42,0.08)]'
                }`}
              >
      ...
      {selectedOrder ? (
        <ClientPanel className="p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.18em] text-slate-400">{t('orders.selected_order')}</p>
          ...
          <div className="mt-5 space-y-3">
            {(selectedOrder.items || []).map((item) => (
              <div key={`${selectedOrder.id}-${item.product_id}-${item.product_name}`} className="rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3">
                <div className="flex items-start justify-between gap-4">
                  <div>
