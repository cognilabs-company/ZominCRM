      ...
      {data?.summary ? (
        <div className="grid grid-cols-2 gap-3">
          <ClientPanel className="p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">{t('bottles.outstanding')}</p>
            <p className="mt-2 text-2xl font-semibold text-slate-950">{data.summary.total_outstanding_bottles_count}</p>
          </ClientPanel>
          <ClientPanel className="p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-400">{t('bottles.deposit_held')}</p>
            <p className="mt-2 text-2xl font-semibold text-slate-950">{formatAmount(getBottleDepositHeldTotal(data.summary), language)}</p>
          </ClientPanel>
        </div>
      ) : null}
      ...
      <ClientPanel className="p-5">
        ...
        <div className="mt-4 space-y-3">
          {data?.balances?.length ? data.balances.map((balance) => (
            <div key={balance.id} className="rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-slate-950">{balance.product_name}</p>
                  <p className="mt-1 text-sm text-slate-500">{balance.product_size_liters ? `${balance.product_size_liters}L` : '-'}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-slate-950">{t('bottles.count_label', { count: balance.outstanding_bottles_count })}</p>
                  <p className="mt-1 text-xs text-slate-500">{formatAmount(balance.deposit_held_uzs, language)}</p>
                </div>
              </div>
            ...
            <div className="rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-500">{t('bottles.no_balances')}</div>
          )}
        </div>
      </ClientPanel>
      ...
      <ClientPanel className="p-5">
        ...
        <div className="mt-4 space-y-3">
          {data?.movements?.length ? data.movements.map((movement) => (
            <div key={movement.id} className="rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-slate-950">{getMovementLabel(movement.movement_type, language)}</p>
                  <p className="mt-1 text-sm text-slate-500">
                    {movement.product_name || '-'}
                    {movement.product_size_liters ? ` / ${movement.product_size_liters}L` : ''}
                  </p>
                  <p className="mt-1 text-xs text-slate-500">{movement.order_id ? formatOrderRef(movement.order_id) : t('bottles.no_order_reference')}</p>
                </div>
                <div className="text-right">
                  ...
                  <p className="mt-1 text-xs text-slate-500">{t('bottles.delta_label', { count: getBottleMovementCount(movement) })}</p>
                </div>
              </div>
            ...
            <div className="rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-500">{t('bottles.no_movements')}</div>
          )}
        </div>
      </ClientPanel>
    </ClientPage>
